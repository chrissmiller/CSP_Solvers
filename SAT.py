...
class SAT:
    ...
    def gsat(self):
        for i in range(self.iteration_limit):
            ...
            if random.random() > self.threshold:
                #print("Iteration " + str(i) + " rand flip")
                var_index = random.randint(1,self.num_vars)

                self.values[var_index] = not self.values[var_index]
            else:
                #print("Iteration " + str(i) + " best flips")
                to_flip = self.get_best_flip()
                if to_flip < 0:
                    to_flip = -to_flip
    ...
    def walksat(self):
        for i in range(self.iteration_limit):
            ...
            # minor performance improvement for 1 clause, prevents double eval
            if len(clause) == 1:
                self.values[clause[0]] = not self.values[clause[0]]
                continue
